...
본 프로젝트를 통해 개발하고자 하는 ClubLens는 사진 동아리의 복잡한 운영 업무(지원자 관리, 정식 부원 승격, 회비 장부, 출사 조 편성)를 효율적으로 통합 관리하는 소프트웨어이다. 본 보고서는 시스템 요구사항을 분석하여 '시스템이 무엇을 하는가'를 정의하고, 객체 간의 유기적인 관계를 Domain 및 Use Case 분석을 통해 상세히 기술한다. 특히 사용자의 편의성을 극대화한 UI 설계(Boundary )와, 객체 직렬화 및 BCE 패턴을 활용한 효율적인 아키텍처를 제안함으로써 실무적인 동아리 관리 솔루션을 제시하고자 한다.
...
지난 개념화 단계에서는 시스템의 주요 이해관계자인 부원과 지원자를 액터로 고려하였으나, 본 시스템이 관리자 전용 로컬 소프트웨어라는 점을 명확히 하기 위해 실제 시스템을 직접 조작하는 Administrator 만을 유일한 액터 로 확정하여 설계를 정교화하였다. 부원과 지원자는 시스템 내부에서 관리되는 Entity 로 정의하였다.
